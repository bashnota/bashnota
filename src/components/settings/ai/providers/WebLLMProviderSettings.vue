...
const selectedModel = computed(() => {
  return webLLMModels.value.find(model => model.id === selectedModelId.value)
})
...
const modelOptions = computed(() => {
  const small = webLLMModels.value.filter(model => getModelCategory(model.size) === 'small')
  const medium = webLLMModels.value.filter(model => getModelCategory(model.size) === 'medium')
  const large = webLLMModels.value.filter(model => getModelCategory(model.size) === 'large')

  return [
    ...small.map(model => ({
      value: model.id,
      label: isModelDownloaded.value && downloadedModels.value.includes(model.id) ? `${model.name} (Downloaded)` : model.name,
...
const getModelName = (modelId: string): string => {
  const model = webLLMModels.value.find(m => m.id === modelId)
...
onMounted(async () => {
  ...
  if (webLLMModels.value.length > 0 && !selectedModelId.value) {
    const smallModel = webLLMModels.value.find(m => getModelCategory(m.size) === 'small')
