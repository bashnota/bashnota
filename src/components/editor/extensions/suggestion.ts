...
function createAdvancedCommands(): CommandItem[] {
  ...
  B --> D[Server2]`
    ...
    },
    {
      ...
      command: ({ editor, range }: CommandArgs) => {
        const notaId = router.currentRoute.value.params.id;
        const citationStore = useCitationStore();
        
        editor.chain().focus().deleteRange(range).run();
        
        // Based on the citationStore API, we need to manually handle citations
        // since openCitationDialog doesn't exist in the store
        const citations = citationStore.getCitationsByNotaId(notaId as string);
        
        if (citations.length === 0) {
          toast('No citations available. Add citations in the References panel first.');
        } else {
          // Open our own custom citation dialog
          // This would typically be handled by a separate component
          toast('Citation feature will be implemented in a future version');
        }
      },
    },
    {
